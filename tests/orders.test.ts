...
// Gina sert à midi en semaine : on fixe l'heure au jeudi midi pour ne pas dépendre de l'horloge.
const SPOT = 'gina';
...
describe('cycle de vie d’une commande', () => {
  it('réserve, règle le rusher et rend le reste', () => {
    ...
    orders.pickUp(order.id, bob.id, 2050);
  ...
  it('refuse un article absent du menu du spot', () => {
    const alice = user();
    fund(alice.id, 2000);
    const status = code(() =>
  ...
  it('réserve le prix publié le plus élevé d’un menu EPFL, et refuse le prix au poids', () => {
    const alice = user();
    fund(alice.id, 3000);
    const menu = [
      {
        title: 'Aujourd’hui',
        items: [
          { id: 'e-menu', name: 'Menu du jour', priceCents: 1240, tiers: { student: 860, doctoral: 990, campus: 1100, visitor: 1240 } },
          { id: 'e-buffet', name: 'Buffet', priceCents: 0, per100gCents: 290 },
        ],
      },
    ];
